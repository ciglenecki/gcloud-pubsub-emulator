// Creates topics and subscriptions based on PUBSUB_PROJECT1 os.env
	...
	for topicID, subscriptions := range topics {
		...
		for _, subscription := range subscriptions {
			...
			if len(subscriptionParts) == 2 {
				...
				debugf("    	with endpoint %q", pushEndpoint)
			...
			} else if len(subscriptionParts) == 1 {
				...
				return fmt.Errorf("Hostname syntax is incorrect. Only one hostname is allowed.")
			}
		}
	}
...
func main() {
	flag.Parse()
	flag.Usage = func() {
		fmt.Printf(`Usage: env PUBSUB_PROJECT1="project1,topic1,topic2:subscription1,topic3:subscription2+endpoint1" %s`+"\n", os.Args[0])
		flag.PrintDefaults()
	}
